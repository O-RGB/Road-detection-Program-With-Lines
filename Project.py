...
model=torch.load('Road_model2.pth',map_location ='cpu')
model.eval()
...
def ton(img,number):
    ...
    ##ทำ Canny
    dst = cv.Canny(img, ret, 200, None, 3)
    
    ##ทำเส้น Canny ให้สมูท
    kernel = cv2.getStructuringElement(cv2.MORPH_RECT,(3,3))
    dilation = cv2.dilate(dst,kernel,iterations = 4)
    ...
    ##Hough 
    lines = cv.HoughLines(dst, 1, np.pi / 360, 150, None, 0, 0)
    arraytest = []
    kmeanP1  =  []
    kmeanP2  =  []
    ##Hough ขีดเส้น
    if lines is not None:
        for i in range(0, len(lines)):
            ...
            x0 = a * rho
            y0 = b * rho
            pt1 = (int(x0 + 1300*(-b)), int(y0 + 1500*(a)))
            pt2 = (int(x0 - 1300*(-b)), int(y0 - 1500*(a)))
            try:
            ...
            if (sun > 0.25 and sun < 0.8):
                ...
                ##วาดเส้นลงรูป
                cv.line(dst, pt1, pt2, (255,0,255), 2, cv.LINE_AA)
...
i = 15
while True:
    cap = cv2.VideoCapture("2021-04-06 22-24-13.mp4",0) 
    cap.set(1,i); 
    eat, img = cap.read()
    height, width = img.shape[:2]
    img = cv2.resize(img, (800,500), interpolation = cv2.INTER_AREA)
    ##ครอบภาพด้วยตัวเองภาพทดสอบ 1
    ##cropped_image = img[800 : height, int(width/4)+200 : width-int(width/4)-200]

    ##ครอบภาพด้วยตัวเองภาพทดสอบ 2
    ##cropped_image = img[300 : height-200, int(width/3)-150 : width-int(width/2)+250]
    cropped_image = img
    ##ส่งภาพที่ครอบแล้วไปขีด Hougn
    cdst = ton(cropped_image,i)
    ...
    ##สำหรับ save ทดสอบ
    ##cv2.imwrite("New folder/img_%d_%s_1.jpg" % (i,classesFile[index]), cropped_image)
    cv2.imwrite("New folder/img_%d_%s_1.jpg" % (i,classesFile[index]), cdst)
    ##cv2.imwrite("New folder/img_%d_%s_2.jpg" % (i,classesFile[index]), img2)
